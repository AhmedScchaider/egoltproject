...
?>
<div>
<form action="<?php echo JRoute::_('index.php?option=com_egoltproject&view=compats'); ?>" method="post" name="adminForm">
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th width="1%" class="nowrap">
					<?php echo JHtml::_('grid.sort',  'COM_EGOLTPROJECT_ID', 'lang.compat_id', $listDirn, $listOrder); ?>
				</th>
			</tr>
		</thead>
		<tfoot>
		...
		<?php foreach ($this->items as $i => $item): ?>
			<tr class="row<?php echo $i % 2; ?>">
				<td class="center">
					<?php echo JHtml::_('grid.id', $i, $item->compat_id); ?>
				</td>
				<td>
					<a href="<?php echo JRoute::_('index.php?option=com_egoltproject&task=compat.edit&id='.(int) $item->compat_id. '&elang='. $item->lang_code); ?>">
							<?php echo $this->escape($item->name); ?>
					</a>
					<?php if($item->imageurl) :?>
					<p class="smallsub">
						<img
						src="<?php echo JURI::root() . $this->params->get('compats_dir', 'media/egoltproject/compats') . '/' . $item->imageurl;?>"
				...
				<?php foreach($this->langs as $i => $languages): ?>
					...
					?>
					<a href="<?php echo JRoute::_('index.php?option=com_egoltproject&task=compat.edit&id='. $item->compat_id . '&elang=' . $languages->lang_code); ?>">
				...
				<td class="center">
					<?php echo JHTML::_('jgrid.published', $item->published, $i, 'compats.'); ?>
				</td>
				<td class="center">
					<?php echo (int) $item->compat_id; ?>
				</td>
			</tr>
			<?php endforeach; ?>
		</tbody>
