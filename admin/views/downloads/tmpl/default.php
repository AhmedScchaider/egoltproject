...
<form action="<?php echo JRoute::_('index.php?option=com_egoltproject&view=downloads'); ?>" method="post" name="adminForm">
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th width="10%">
					<?php echo JTEXT::_('COM_EGOLTPROJECT_DL_HIT'); ?>
				</th>
				<th width="5%">
					<?php echo JHtml::_('grid.sort',  'COM_EGOLTPROJECT_HIT', 'general.hit', $listDirn, $listOrder); ?>
				</th>
				<th width="1%" class="nowrap">
					<?php echo JHtml::_('grid.sort',  'COM_EGOLTPROJECT_ID', 'lang.download_id', $listDirn, $listOrder); ?>
				</th>
			</tr>
		</thead>
		<tfoot>
		...
		<?php foreach ($this->items as $i => $item): ?>
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td>
					<a href="<?php echo JRoute::_('index.php?option=com_egoltproject&task=download.edit&id='.(int) $item->download_id. '&elang='. $item->lang_code); ?>">
				...
				<?php foreach($this->langs as $i => $languages): ?>
					...
					?>
					<a href="<?php echo JRoute::_('index.php?option=com_egoltproject&task=download.edit&id='. $item->download_id . '&elang=' . $languages->lang_code); ?>">
				...
				<td class="center">
					<?php echo JHTML::_('jgrid.published', $item->published, $i, 'downloads.'); ?>
				</td>
				<td class="center">
					<?php echo JHTML::_('date', $item->pubdate, JText::_('DATE_FORMAT_LC')); ?>
				</td>
				<td align="center">
					<?php echo $item->dl_hit; ?>
				</td>
				<td align="center">
					<?php echo $item->hit; ?>
				</td>
				<td class="center">
					<?php echo (int) $item->download_id; ?>
				</td>
			</tr>
			<?php endforeach; ?>
		</tbody>
