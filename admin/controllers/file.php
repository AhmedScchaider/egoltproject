...
// require helper file
JLoader::register('UploaderHelper', JPATH_ADMINISTRATOR . '/components/com_egoltproject/helpers/uploader.php');

$language = JFactory::getLanguage();
$language->load('com_media', JPATH_ADMINISTRATOR);

class EgoltProjectControllerFile extends JController
{
	/**
	 * Upload a file
	 *
	...
	function upload()
	{
		...
		if (isset($file['name']))
		{
			...
			$err = null;
			$ftype = JRequest::getVar('ftype', 'image');
			if (!UploaderHelper::canUpload($file, $ftype, $err))
			{
	...
	function delete()
	{
		...
		if ($tmpl == 'component') {
			...
			$this->setRedirect('index.php?option=com_egoltproject&view=uplodaer&folder='.$folder.'&tmpl=component');
		} else {
			$this->setRedirect('index.php?option=egoltproject&folder='.$folder);
		}
			...
			if (count($paths))
			{
				...
				foreach ($paths as $path)
				{
					...
					if (is_file($fullPath))
					{
						...
						$result = $dispatcher->trigger('onContentBeforeDelete', array('com_egoltproject.file', &$object_file));
						if (in_array(false, $result, true)) {
							// There are some errors in the plugins
						...
						$dispatcher->trigger('onContentAfterDelete', array('com_egoltproject.file', &$object_file));
