...
class EgoltProjectModelRevs extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select('g.*');
		$query->from('#__egoltproject_reviews as g');

		// Join over the language Content
		$query->select('dl.title, dl.download_id');
		$query->join('LEFT', '`#__egoltproject_downloads_lg` AS dl ON dl.download_id = g.download_id');
		...
		if (!empty($search)) {
			if (stripos($search, 'id:') === 0) {
				...
				$query->where('(dl.title LIKE '.$search.') OR (g.comment LIKE '.$search.') OR (g.commenter LIKE '.$search.') OR (g.email LIKE '.$search.')');
			}
		}

		// Filter on the language.
		if ($language = $this->getState('filter.language')) {
			$query->where('g.lang_code = ' . $db->quote($language));
		}
