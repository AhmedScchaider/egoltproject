...
class EgoltProjectModelDownloads extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		parent::populateState('lang.download_id', 'DESC');
	}
	...
	protected function getListQuery()
	{
		...
		$query->select('lang.download_id as id');
		...
		if (!empty($search)) {
			if (stripos($search, 'id:') === 0) {
				...
				$query->where('(lang.title LIKE '.$search.' OR general.alias LIKE '.$search.' OR lang.notes LIKE '.$search.')');
			}
		}
	...
	function getDLHit($download_id)
	{
		$db = JFactory::getDBO();
		$query = $db->getQuery(true);
		$query->select('id,download_id ');
		$query->from('#__egoltproject_logs');
