...
class JFormFieldFilemanager extends JFormField
{
	...
	protected function getInput()
	{
		...
		if($this->element['ftype'])
			$ftype = (string) $this->element['ftype'];
		else
			$ftype = 'image';		
		...
		// }
		// elseif (file_exists(JPATH_ROOT . '/' . $directory))
		// {
			$folder = $directory;
		// }
		// else
		...
		$html[] = '		<a class="modal" title="' . JText::_('JLIB_FORM_BUTTON_SELECT') . '"' . ' href="'
			...
			: ($link ? $link
				: 'index.php?option=com_egoltproject&amp;view=uploader&amp;tmpl=component&amp;asset=' . $asset . '&amp;author='
				. $this->form->getValue($authorField)) . '&amp;fieldid=' . $this->id . '&amp;folder=' . $folder . '&amp;ftype=' . $ftype) . '"'
